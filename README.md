...
```bash
docker pull ghcr.io/dennis-au/ovirt_stocktake:v0.1.24
```
...
Each GitHub release includes an `ovirt-inventory-compose-v0.1.24.tar.gz` deployment bundle containing `compose.yaml`, empty `inventory_data/` and `postgres_data/` folders, `.env.example`, `setup.sh`, and this README. Download, extract, generate the editable `.env`, then start the app:

```bash
curl -LO https://github.com/dennis-au/ovirt_stocktake/releases/download/v0.1.24/ovirt-inventory-compose-v0.1.24.tar.gz
tar -xzf ovirt-inventory-compose-v0.1.24.tar.gz
cd ovirt-inventory-compose-v0.1.24
./setup.sh
