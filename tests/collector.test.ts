...
function ovirtFetchMock(
  ...
  return vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (resource === "hosts") {
      const hosts = options.hostList
        ? Array.isArray(options.hostList)
          ? options.hostList
          : [options.hostList]
        : [
            {
              ...
              certificate: { content: "certificate-content-must-not-persist" }
            }
          ];
      return jsonResponse({ host: hosts });
    }
...
function expandedOvirtFetchMock(
  ...
  const snapshotListDate = options.snapshotListDate === undefined ? "2026-08-10T10:00:00.000Z" : options.snapshotListDate;
  const snapshotDetailDate = options.snapshotDetailDate === undefined ? "2026-08-10T10:00:00.000Z" : options.snapshotDetailDate;
  ...
  return vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (resource === "hosts") {
      return jsonResponse({
        host: [{ id: "host-1", name: "host-1", cluster: { id: "cluster-1" }, status: "up", certificateExpiresAt: "2036-08-15T03:11:33.000Z" }]
      });
    }
    if (resource === "vms") {
      return jsonResponse({
    ...
    if (parsed.pathname.includes("/snapshots/") && resource !== "snapshots") {
      if (options.snapshotDetailStatus) {
        return jsonResponse({}, options.snapshotDetailStatus);
      }
      return jsonResponse({
        snapshot: {
          id: resource,
          description: options.snapshotDescription ?? "pre-change",
          ...(options.snapshotType ? { snapshot_type: options.snapshotType } : {}),
          ...(snapshotDetailDate ? { date: snapshotDetailDate } : {})
        }
      });
    }
    if (resource === "snapshots") {
        ...
        snapshot: [
          {
            ...
            description: options.snapshotDescription ?? "pre-change",
            ...(options.snapshotType ? { snapshot_type: options.snapshotType } : {}),
            ...(snapshotListDate ? { date: snapshotListDate } : {})
          }
        ]
      });
    }
...
describe("oVirt backend collector", () => {
  ...
  it("hydrates a missing snapshot list date from the read-only snapshot detail endpoint", async () => {
    const fetchMock = expandedOvirtFetchMock({
      snapshotListDate: null,
      snapshotDetailDate: "2026-08-10T10:00:00.000Z"
    });
  ...
  it("keeps collection evidence when a required snapshot detail request fails", async () => {
    const fetchMock = expandedOvirtFetchMock({ snapshotListDate: null, snapshotDetailStatus: 500 });
    const snapshot = await collectOvirtSnapshot(target, { fetchImpl: fetchMock as unknown as typeof fetch });

    expect(snapshot.status).toBe("partial");
    expect(snapshot.resources.vmSnapshots).toEqual([
      expect.objectContaining({ id: "snapshot-1", vm: { id: "vm-1", name: "api-01" } })
    ]);
    expect(snapshot.errors).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ resource: "vmSnapshots", message: expect.stringContaining("detail collection failed") })
      ])
    );
  });

  it("keeps an undated snapshot and warning when its detail response also lacks a date", async () => {
    const fetchMock = expandedOvirtFetchMock({ snapshotListDate: null, snapshotDetailDate: null });
    const snapshot = await collectOvirtSnapshot(target, { fetchImpl: fetchMock as unknown as typeof fetch });

    expect(snapshot.status).toBe("success");
    expect(snapshot.resources.vmSnapshots).toEqual([
      expect.objectContaining({ id: "snapshot-1", description: "pre-change", vm: { id: "vm-1", name: "api-01" } })
    ]);
    expect(snapshot.warnings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ resource: "vmSnapshots", message: expect.stringContaining("has no creation date") })
      ])
    );
  });

  it("does not hydrate an active snapshot identified by its semantic type", async () => {
    const fetchMock = expandedOvirtFetchMock({
      snapshotListDate: null,
      snapshotType: "active",
      snapshotDescription: "Current state"
    });
